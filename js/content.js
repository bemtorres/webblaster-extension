(function () {
  ...
  if (window.webEraserGameLoaded) return;
  window.webEraserGameLoaded = true;
  ...
  const SCORING = {
    ...
    'TABLE': 2500, 'TR': 500, 'TD': 200, 'TH': 200,
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'TOGGLE_GAME') {
      toggleGame(request.config);
      sendResponse({ isActive: state.active });
  ...
  function enableGame(config) {
    ...
    state.isPaused = true; // Pause for countdown
    ...
    // Start Sequence
    startCountdown(() => {
      if (!state.active) return; // In case user stopped during countdown
      state.isPaused = false;
      state.startTime = Date.now(); // Reset start time after countdown
      state.elapsedSeconds = 0;
      addListeners();
      startGameLoop();
      startBossSpawner();
      playMusic();
      console.log("WEBBLASTER: GAME ON");
    });
  }
  ...
  function showSummaryModal() {
    ...
    // In Timed Mode, show the total time limit
  ...
  function createHUD() {
    ...
    hudDiv.innerHTML = `
            ...
            <div class="wb-hud-right">
                ...
                <button id="wb-stop-btn" class="wb-btn-stop">STOP</button>
            </div>
    ...
    document.getElementById('wb-stop-btn').addEventListener('click', (e) => {
      e.stopPropagation();
      toggleGame();
    });
  ...
  function updateHoverVisuals(e) {
    ...
    if (e.shiftKey) {
      ...
      // Fallback to direct parent if no structural found
  ...
  function onClick(e) {
    ...
    if (e.shiftKey) {
      // Find closest structural parent logic (same as hover)
      let curr = target.parentElement;
      let parent = null;
      while (curr && curr.tagName !== 'BODY' && curr.tagName !== 'HTML') {
        if (STRUCTURAL_TAGS.includes(curr.tagName)) {
  ...
  function startBossSpawner() {
    // Try to spawn a boss every 5 seconds
    bossInterval = setInterval(spawnBoss, 5000);
  }
  ...
  function spawnBoss() {
    ...
    // Find candidates: specific elements only (+ wb-enemy)
    ...
    const visibleCandidates = Array.from(candidates).filter(el => {
      // If it's a DIV, it must have wb-enemy OR be explicitly wb-boss (though we filter active bosses out)
      ...
      // Must be visible on screen and have decent size
      return (
    ...
    // Apply Boss Status
    const health = Math.floor(Math.random() * 20) + 10; // 10 to 30 HP
    randomEl.classList.add('wb-boss');
    randomEl.dataset.wbHealth = health;
    randomEl.dataset.wbMaxHealth = health;

    spawnFloatingText(randomEl, "WARNING: BOSS DETECTED!", false);
  }

  function hitBoss(el) {
    let health = parseInt(el.dataset.wbHealth);
    let maxHealth = parseInt(el.dataset.wbMaxHealth); // Ensure this is stored on spawn
    health--;
    ...
    const pitch = 0.5 + (ratio * 1.5); // 1 * 1.5 + 0.5 = 2.0 | 0 * 1.5 + 0.5 = 0.5
  ...
  function destroyBoss(el) {
    el.classList.remove('wb-boss'); // Remove glow
  ...
  function playMusic() {
    ...
    if (!bgMusic) {
      ...
      bgMusic = new Audio(chrome.runtime.getURL(track));
      bgMusic.loop = true;
      bgMusic.volume = 0.3; // Background level
    }
  ...
  function playShootSound() {
    ...
    try {
      const audio = new Audio(chrome.runtime.getURL('sounds/shot_fx.mp3'));
      audio.volume = 0.5;
      // Random pitch variation (0.8 to 1.5) to make shots sound unique
      audio.playbackRate = 0.8 + Math.random() * 0.7;
      // Force pitch change on browsers that support this prop
  ...
  function playBossHitSound(pitch) {
    if (!state.soundEnabled) return;
    try {
      const audio = new Audio(chrome.runtime.getURL('sounds/fx_boos.mp3'));
      audio.volume = 0.6;
  ...
  function playBossExplosionSound() {
    if (!state.soundEnabled) return;
    try {
      const audio = new Audio(chrome.runtime.getURL('sounds/fx_explosion.mp3'));
      audio.volume = 0.8;
      audio.play();
    } catch (e) {
  ...
  function destroyElement(el) {
    ...
    if (state.combo > 1) {
      spawnFloatingText(el, `x${state.combo} COMBO!`, true);
    }
  ...
  function updateTimer() {
    ...
    if (state.mode === 'timed') {
      ...
      if (remaining <= 0) {
        ...
        // disableGame will handle the alert now
        disableGame();
      } else {
